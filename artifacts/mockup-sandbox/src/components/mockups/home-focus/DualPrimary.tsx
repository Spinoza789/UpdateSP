  ...
  return (
    <div className="w-full flex flex-col pb-14 min-h-screen" style={{ background: "#F0F4F8", fontFamily: "Inter, system-ui, sans-serif" }}>
      ...
      <main className="flex-1 flex flex-col px-4 pt-4 pb-4 gap-3 max-w-md mx-auto w-full">
        ...
        {/* DUAL PRIMARY — side-by-side, equal weight, equal height */}
        <div className="flex gap-2" style={{ minHeight: "200px" }}>

          {/* Left — Group Buy Login */}
          <div
            className="flex-1 rounded-2xl overflow-hidden relative flex flex-col"
            style={{ background: "linear-gradient(160deg, #0F172A 0%, #0D2550 100%)" }}
          >
            <div className="flex-1 flex flex-col p-4 gap-2 relative z-10 justify-between">
              <div>
                <div className="flex items-center gap-1.5 mb-2">
                  <ShoppingBag className="w-4 h-4 text-blue-400" />
                  <span className="text-[9px] font-bold uppercase tracking-wider text-blue-400">New</span>
                </div>
                <h2 className="text-base font-black text-white leading-tight">Place an order</h2>
                <p className="text-xs text-white/50 mt-1 leading-snug">Browse 126+ products, group buy checkout.</p>
              </div>
              <button
                onClick={() => {}}
                className="h-10 rounded-xl text-xs font-black flex items-center justify-center gap-1.5 text-white w-full"
                style={{ background: "linear-gradient(135deg, #3B82F6, #2563EB)", boxShadow: "0 3px 12px rgba(59,130,246,0.4)" }}
              >
                <Users className="w-3.5 h-3.5" /> Group Buy <ArrowRight className="w-3 h-3" />
              </button>
            </div>
            <div className="absolute top-0 right-0 w-24 h-24 rounded-full pointer-events-none" style={{ background: "rgba(59,130,246,0.14)", transform: "translate(35%,-35%)" }} />
          </div>

          {/* Right — Lonely Vial */}
          <div
            className="flex-1 rounded-2xl overflow-hidden relative flex flex-col"
            style={{ background: "linear-gradient(160deg, #120A2E 0%, #1D0D45 100%)", border: "1px solid rgba(139,92,246,0.25)" }}
          >
            <div className="flex-1 flex flex-col p-4 gap-2 relative z-10 justify-between">
              <div>
                <div className="flex items-center gap-1.5 mb-2">
                  <FlaskConical className="w-4 h-4" style={{ color: "#A78BFA" }} />
                  <span className="text-[9px] font-bold uppercase tracking-wider" style={{ color: "#A78BFA" }}>Vials</span>
                </div>
                <h2 className="text-base font-black text-white leading-tight">The Lonely Vial</h2>
                <p className="text-xs mt-1 leading-snug" style={{ color: "rgba(255,255,255,0.5)" }}>Individual vials, no minimums. Pay with USDT.</p>
              </div>
              <button
                onClick={() => {}}
                className="h-10 rounded-xl text-xs font-black flex items-center justify-center gap-1.5 text-white w-full"
              ...
              >
                <FlaskConical className="w-3.5 h-3.5" /> Browse <ArrowRight className="w-3 h-3" />
              </button>
            </div>
            <div className="absolute top-0 right-0 w-24 h-24 rounded-full pointer-events-none" style={{ background: "rgba(139,92,246,0.10)", transform: "translate(35%,-35%)" }} />
          </div>
        </div>
