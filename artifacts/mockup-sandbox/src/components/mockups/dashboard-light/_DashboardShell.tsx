...
// ─── Stubbed API hooks (mockup — no network) ────────────────────────────────
function useGetProducts(_opts?: unknown) {
  return { data: [] as { id: string; name: string; price: number | string }[], isFetching: false };
}
function useListLabTests(_arg?: unknown, _opts?: unknown) {
  return {
    data: [] as { id: string; peptideName: string; supplier: string; batchCode?: string }[],
    isFetching: false,
  };
}
...
import { palette, ACCENT, ACCENT_SOFT, HERO_GRAD, FONT } from "./_dashboard-theme";
...
export function DashboardShell({
  ...
  return (
    <div data-theme={dark ? "dark" : "light"} className="flex w-full min-h-screen lg:h-screen lg:overflow-hidden" style={{ background: T.page, fontFamily: FONT, color: T.text }}>
      <style>{`
      ...
      {/* HubBottomNav omitted in mockup (desktop-oriented) */}
    </div>
  );
}
