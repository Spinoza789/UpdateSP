...
function TopNav() {
  return (
    <div className="flex items-center justify-between px-4 h-12 shrink-0" style={{ background: "transparent", position: "absolute", top: 0, left: 0, right: 0, zIndex: 10 }}>
      <span className="text-sm font-bold text-white tracking-wide">Salt &amp; Peps</span>
      <div className="flex items-center gap-2">
        <button onClick={() => {}} className="text-xs font-semibold text-white/70 flex items-center gap-1 px-2 py-1 rounded-lg" style={{ background: "rgba(255,255,255,0.12)" }}>
          <User className="w-3 h-3" /> Login
        </button>
        <span className="text-xs font-bold text-white px-2 py-1 rounded-lg" style={{ background: "rgba(59,130,246,0.7)", backdropFilter: "blur(4px)" }}>USD</span>
      </div>
    </div>
  );
}
...
function BottomTabs() {
  return (
    <div className="fixed bottom-0 inset-x-0 flex h-14" style={{ background: "#0F172A", borderTop: "1px solid rgba(255,255,255,0.06)" }}>
      {[
...
export function BleedHero() {
  return (
    <div className="w-full flex flex-col pb-14 min-h-screen" style={{ background: "#F0F4F8", fontFamily: "Inter, system-ui, sans-serif" }}>
      ...
      {/* HERO bleeds full width, behind nav, sharp top corners */}
      <div className="relative w-full" style={{ background: "linear-gradient(160deg, #0F172A 0%, #0D2550 70%, #0F2A70 100%)", minHeight: "300px", borderBottomLeftRadius: "28px", borderBottomRightRadius: "28px" }}>
        <TopNav />
        <div className="px-5 pt-16 pb-8 flex flex-col gap-5 relative z-10">
          {/* Brand inside hero */}
          <div>
            <p className="text-xs font-bold uppercase tracking-widest text-blue-300/70 mb-1">Salt &amp; Peps</p>
            <h2 className="text-3xl font-black text-white leading-tight">Place an order</h2>
            <p className="text-sm mt-2 leading-relaxed" style={{ color: "rgba(255,255,255,0.5)", maxWidth: "260px" }}>Browse 126+ peptide products and submit your order for the next group buy.</p>
          </div>
          <button
            onClick={() => {}}
            className="h-14 rounded-2xl text-base font-black flex items-center justify-center gap-2 text-white w-full"
        ...
        {/* decorative orbs */}
        <div className="absolute top-0 right-0 w-52 h-52 rounded-full pointer-events-none" style={{ background: "rgba(59,130,246,0.13)", transform: "translate(35%,-35%)" }} />
        <div className="absolute bottom-0 left-0 w-32 h-32 rounded-full pointer-events-none" style={{ background: "rgba(59,130,246,0.07)", transform: "translate(-30%,30%)" }} />
      </div>

      {/* REST OF PAGE — contained with padding */}
      <main className="flex-1 flex flex-col px-4 pt-3 pb-4 gap-3 max-w-md mx-auto w-full">

        {/* Lonely Vial */}
        <div
          className="rounded-2xl overflow-hidden relative flex items-center gap-4 px-5"
          style={{ background: "linear-gradient(160deg, #120A2E 0%, #1D0D45 100%)", minHeight: "96px", border: "1px solid rgba(139,92,246,0.28)" }}
        >
        ...
        {/* Secondary list */}
        <div className="rounded-2xl overflow-hidden" style={{ background: "linear-gradient(180deg, #0F1E30 0%, #0D1B28 100%)", border: "1px solid rgba(255,255,255,0.06)" }}>
          {[
