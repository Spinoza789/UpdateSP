      ...
      <div className="approved-dispatch-flow space-y-4 sm:space-y-5">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-0.5 text-[13px]" style={{ color: "var(--t-subtle)" }}>Review dispatched orders, exports and uploaded evidence.</p>
          </div>
          <button type="button" onClick={() => onStateChange({ ...state, view: "desk" })} className="flex h-10 items-center justify-center gap-2 rounded-lg bg-white px-4 text-[13px] font-semibold" style={{ border: `1px solid ${V2_CARD_BORDER}`, color: "var(--t-muted)" }}>
            <ArrowLeft className="h-4 w-4" /> Back to Dispatch
          </button>
        </div>
        {banner && <div className="flex items-center gap-2 rounded-xl border border-green-200 bg-green-50 px-4 py-3 text-[13px] font-semibold text-green-700"><CheckCircle2 className="h-4 w-4" /><span className="flex-1">{banner}</span><button type="button" onClick={() => setBanner(null)} aria-label="Dismiss"><X className="h-3.5 w-3.5" /></button></div>}
    ...
    <div className="approved-dispatch-flow space-y-4 sm:space-y-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-0.5 text-[13px]" style={{ color: "var(--t-subtle)" }}>Select received parcels and dispatch the orders they can fulfil.</p>
        </div>
        <button type="button" onClick={() => onStateChange({ ...state, view: "log" })} className="flex h-10 items-center justify-center gap-2 rounded-lg bg-white px-4 text-[13px] font-semibold" style={{ border: `1px solid ${V2_CARD_BORDER}`, color: "var(--t-muted)" }}>
