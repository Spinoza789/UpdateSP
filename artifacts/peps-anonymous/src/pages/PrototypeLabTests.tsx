...
function SubmitSection() {
  ...
  const handleSubmit = async () => {
    if (!form.url.trim()) { setError("Report URL is required"); return; }
    if (!form.peptideName.trim()) { setError("Compound name is required"); return; }
    setSubmitting(true); setError("");
    try {
      const res = await fetch("/api/lab-tests/submit", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          url: form.url.trim(),
  ...
  if (success) {
    return (
      <div className="flex flex-col items-center justify-center gap-4 py-16 px-6 text-center">
        ...
        <button
          onClick={() => { setSuccess(false); setForm({ url: "", peptideName: "", supplier: "", labName: "Janoshik", testType: "", productCategory: "", isThirdPartyTest: true, batchCode: "", notes: "", submittedBy: "", mgAmount: "", massUnit: "mg", purityPct: "", endotoxinEuMg: "", sterilityPass: "", testDate: "", heavyMetalArsenic: "", heavyMetalCadmium: "", heavyMetalLead: "", heavyMetalMercury: "" }); }}
  ...
  return (
    <div className="max-w-lg mx-auto px-4 py-6 space-y-5">
      <div>
        ...
        <p className="text-sm mt-1" style={{ color: "var(--t-subtle)" }}>
          Have a lab report URL? Share it with the community. Your submission will be reviewed before going live.
      ...
      <div className="rounded-2xl p-4 space-y-4 border" style={{ background: "var(--t-surface)", borderColor: "var(--t-border)" }}>
        <div>
          ...
          {extractNote && (
            <div className={`mt-2 flex items-start gap-2 px-3 py-2 rounded-lg text-xs font-medium ${extractNote.ok ? "text-emerald-700" : "text-red-600"}`}
      ...
      <button
        ...
        disabled={submitting || !form.url.trim() || !form.peptideName.trim()}
...
export default function PrototypeLabTests({ bare }: { bare?: boolean } = {}) {
