...
- [Wholesale share status races](wholesale-share-status-races.md) — lock/cancel/submit/delivery writes must each do a CONDITIONAL status-gated update (→409) or they clobber each other; organiser picks WHO receives, but only the designated recipient may set a one-off delivery address.
