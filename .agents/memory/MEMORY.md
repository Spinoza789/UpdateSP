...
- [Wholesale share status races](wholesale-share-status-races.md) — lock/cancel/submit must each do a CONDITIONAL in-transaction status update (gated on expected status, sentinel→409) or they clobber each other; delivery address is server-read from the chosen member's account, never the request body.
