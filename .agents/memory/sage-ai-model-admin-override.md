...
**How to apply:** keep the database setting, primary/fallback model environment values, and source defaults aligned to `gpt-5.5`. When checking providers, test both with GPT-5.5 and distinguish endpoint credential failures from model availability.
